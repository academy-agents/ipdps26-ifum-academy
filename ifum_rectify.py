...
class Rectify():
    ...
    def rectify(self,arc_or_data="arc") -> None:
        ...
        for i in range(centers.shape[0]):
            ...
            poly_sig_mask, fit = ifum_utils.sigma_clip(masks_l[bad_fits],
                                                       centers[i][bad_fits],
            ...
            if np.all(np.sum(bad_fits_s, axis=1) > bad_fits_s.shape[0]/2) and error_from_basic_fit < 3: # np.all(np.sum(~bad_fits_s, axis=1) <= 3*len(self.bad_mask))
                ...
                for j in range(self.mask_groups):
                    ...
                    group_mask, segment_fit = ifum_utils.sigma_clip(masks_split[j][bad_fits_s[j]],
        ...
        for mask in masks_l:
            ...
            _, fit = ifum_utils.sigma_clip(full_shifts[:,mask-1],full_shifts[:,0]-full_shifts[:,mask-1],
    ...
    def _viz_wl(self,prelim_calib=None) -> None:
        ...
        m = 1
        intensity_arc = ifum_utils.get_spectrum_simple_withnan(arc_data,arc_mask_data,m)
        intensity = ifum_utils.get_spectrum_simple_withnan(data,mask_data,m,fits.open(self.cmraymask)[0].data)
        x = np.arange(data.shape[1])
