...
class Stack():
    ...
    def hex_to_grid(self):
        centers_,percentages_,plot_args_ = ifum_hexagonify.hex_grid(**self.run_params)
        ...
        ifum_hexagonify.plot_grid(**plot_args_)
        print("plotting...")
        plt.tight_layout()
        ax.set_aspect('equal')
        plt.show(block=False)
    ...
    def wcs_datacubes(self):
        ...
        for i,npzfile in enumerate(self.npzfiles):
            ...
            colors = plt.get_cmap('gray')(ifum_utils.normalize(hex_medians))
        ...
        for i,npzfile in enumerate(self.npzfiles):
            ...
            for coord in shifted_coords:
                ...
                if len(nearby_hexes)==0:
                    ...
                    nearby_weights = ifum_utils.normalize(hex_medians[nearby_hexes])
        ...
        for i,npzfile in enumerate(self.npzfiles):
            ...
            calc_radecs = np.array([ifum_utils.xy_to_radec(result.x,[xys,xys,radecs])]).T
            # print(result.x)
            ...
            wcs_hexes = []
            for hex in hexes:
                wcs_hex = np.array(ifum_utils.xy_to_radec(result.x,[np.array(list(hex.exterior.coords)),xys,radecs])).T              
        ...
        for i,npzfile in enumerate(self.npzfiles):
            ...
            wcs_pixels = ifum_hexagonify.pxs_from_px(xv,yv,dx,dy)
        ...
        for idx,intensity in enumerate(avg_intensities):
            ...
            for i,line in enumerate(self.isol_sky_lines):
                ...
                try:
                    ...
                    5/3]
                    popt,pcov = scipy.optimize.curve_fit(ifum_utils.gauss_background,
                                                            wl[mask_area],
