...
class Mask():
    ...
    def first_guess(self, deg) -> np.ndarray:
        flat_data = fits.open(self.flatdir)[0].data
        flat_data = scipy.ndimage.median_filter(flat_data,size=(1,5))
        ...
        for i,mask_dots in enumerate(mask_peaks.T):
            ...
            # plt.scatter(x,mask_dots)
            # plt.plot(x,np.poly1d(cont_fit)(x))
            ...
            for i,mask_group in enumerate(masks_split):
                ...
                for mask in mask_group:
                    if mask not in self.bad_mask:
                        ...
                        lbounds.append(np.poly1d(mask_polys[mask])(x)-2)
    ...
    def create_mask(self,sig_mult,mode="flat") -> None:
        if mode=="flat":
            npzfile = np.load(self.datadir)
    ...
    def optimize_trace(self,filename,sig_mult,cmrays=False,expected_peaks=30) -> None:
    ...
    def get_rots(self,arcfilename,datafilename) -> None:
